...
const severityColors: Record<'HIGH' | 'MEDIUM' | 'LOW' | 'NONE', string> = {
  HIGH: 'bg-red-600/25 text-red-200 border-red-500/50 shadow-[0_0_18px_rgba(248,113,113,0.55)] animate-pulse',
  MEDIUM:
...
export const StatusBadge: React.FC<{ severity?: number | null; label?: string }> = ({
  ...
  const level =
    severity == null ? 'NONE' : severity >= 5 ? 'HIGH' : severity >= 3 ? 'MEDIUM' : 'LOW';
  return (
    <span
