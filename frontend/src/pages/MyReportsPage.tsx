...
import { MapPin, Shield, Sparkles, Clock, ChevronDown, ChevronUp } from 'lucide-react';
...
const MyReportsPage: React.FC = () => {
  ...
  const toggleExpand = async (id: number) => {
    ...
    try {
      ...
      setIncidents((prev) =>
        prev.map((i) => (i.id === id ? { ...i, timeline: res.data.logs || [] } : i)),
      );
    } catch (e) {
      console.error(e);
    }
          ...
          <AnimatePresence>
            {incidents.map((incident, idx) => (
              <motion.div
                ...
                className={`card bg-base-100 shadow-lg border-l-4 overflow-hidden relative group transition-all duration-300 ${
                  expandedId === incident.id ? 'ring-2 ring-base-content/5' : ''
                }`}
                style={{
                  borderLeftColor:
                    (incident.severityScore || 0) >= 4
                      ? '#ef4444'
                      : (incident.severityScore || 0) >= 3
                        ? '#eab308'
                        : '#3b82f6',
                }}
              >
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-xs text-base-content/60">
                        ...
                        <span className="flex items-center gap-1">
                          <MapPin className="w-3 h-3" /> Lat: {incident.latitude?.toFixed(3)}
                        </span>
                        <span className="flex items-center gap-1">
                          <Sparkles className="w-3 h-3 text-secondary" /> AI Cat:{' '}
                          {incident.category}
                        </span>
                      </div>
                    </div>
                    ...
                    {expandedId === incident.id && (
                      ...
                      >
                        <div className="p-6 grid md:grid-cols-2 gap-8">
                          <div>
                            <h4 className="text-xs font-bold uppercase text-base-content/40 mb-3 tracking-widest">
                              Operation Status
                            </h4>
                            {/* Stepper */}
                            <ul className="steps w-full text-xs">
                              <li
                                className={`step ${getStatusStep(incident.status) >= 0 ? 'step-primary' : ''}`}
                              >
                                Reported
                              </li>
                              <li
                                className={`step ${getStatusStep(incident.status) >= 1 ? 'step-primary' : ''}`}
                              >
                                Reviewed
                              </li>
                              <li
                                className={`step ${getStatusStep(incident.status) >= 2 ? 'step-primary' : ''}`}
                              >
                                Dispatched
                              </li>
                              <li
                                className={`step ${getStatusStep(incident.status) >= 3 ? 'step-primary' : ''}`}
                              >
                                Resolved
                              </li>
                            </ul>
